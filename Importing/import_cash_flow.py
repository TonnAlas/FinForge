"""
Cash Flow Statement Data Importer

DESCRIPTION:
This module imports cash flow statement data from Parquet storage files into the Excel 
cash flow sheet. Handles multiple stock tickers with proper data organization
and Excel integration. Replicates functionality of xlwings backup but uses Parquet data.

INPUTS:
- Excel workbook: FinForge.xlsm with "cash flow" sheet
- Ticker symbols: Located in row 4 of the Excel sheet
- Parquet files: Cash flow data from Yahoo Finance (wide format, converted to long)
- Terminal window settings: Selected cash flow line items

OUTPUTS:
- Updated "cash flow" Excel sheet with financial data
...
- Writes to: FinForge.xlsm "cash flow" sheet
...
# Default Yahoo-style order template for cash flow line items
YAHOO_CASH_FLOW_ORDER = [
...
class CashFlowImporter:
    def __init__(self):
        ...
        self.sheet_name = "Cash Flow"
        self.data_type = "cash_flow"
    ...
    def import_data(self, workbook_path=None):
        """
        Import cash flow statement data from Parquet storage to Excel sheet
        """
        print(f"=== Starting Cash Flow Import ===")
        ...
        try:
            ...
            for col_index in range(2, max_col + 1):  # Start from column 2 (skip column A)
                ...
                # Cash flow data is stored in wide format, convert to long
                ...
                # Check if data is in wide format (dates as columns) and convert to long
                if 'line_item' not in df.columns and 'index' in df.columns:
                    df = convert_fundamental_wide_to_long(df)
                
                if df.empty:
                    print(f"  No cash flow data found for {ticker} after conversion")
                    self._mark_column_unavailable(ws, col_index, "No data found")
                    continue
                
                required_cols = {"line_item", "statement_date", "value"}
                if not required_cols.issubset(set(df.columns)):
                    print(f"  Invalid cash flow format for {ticker}")
                ...
                print(f"  Retrieved {len(stmt_data)} cash flow items for '{statement_date}'")
            ...
            print(f"\n✅ Cash Flow Import completed successfully")
            print(f"📊 Successfully processed {success_count} ticker columns")
            
        except Exception as e:
            print(f"❌ Error during cash flow import: {str(e)}")
            import traceback
            traceback.print_exc()
            raise
    ...
    def _get_valid_items_and_populate_column_a(self, ws, wb):
        ...
        if not default_items:
            ...
            if df.empty:
                # Try GM as secondary fallback
                df = self.data_manager.get_fundamental_data('GM', self.data_type)
            if not df.empty and 'line_item' not in df.columns and 'index' in df.columns:
                df = convert_fundamental_wide_to_long(df)
        ...
        line_items = self._get_selected_items_from_terminal_settings("cashFlow", default_items)
    ...
    def _load_terminal_settings(self):
        ...
        settings["cashFlow"] = default_settings["cashFlow"].copy()
        ...
        if isinstance(loaded, dict):
            ...
            cash_flow_settings = loaded.get("cashFlow")
            if isinstance(cash_flow_settings, dict):
                settings["cashFlow"].update(cash_flow_settings)
    ...
    def _apply_yahoo_order(self, available_items):
        ...
        ordered = [item for item in YAHOO_CASH_FLOW_ORDER if item in available_items]
