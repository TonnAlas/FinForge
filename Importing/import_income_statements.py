"""
Income Statement Data Importer

DESCRIPTION:
This module imports income statement data from Parquet storage files into the Excel 
income statements sheet. Handles multiple stock tickers with proper data organization
and Excel integration. Replicates functionality of xlwings backup but uses Parquet data.

INPUTS:
- Excel workbook: FinForge.xlsm with "income statements" sheet
- Ticker symbols: Located in row 4 of the Excel sheet
- Parquet files: Income statement data from Yahoo Finance
- Settings sheet: Custom configuration for INDEX items

OUTPUTS:
- Updated "income statements" Excel sheet with financial data
...
- Called by: Excel VBA ImportIncomeStatements() macro
- Writes to: FinForge.xlsm "income statements" sheet
...
class IncomeStatementImporter:
    def __init__(self):
        ...
        self.data_type = "income_statement"
    ...
    def import_data(self):
        ...
        """
        print(f"=== Starting Income Statement Import ===")
        ...
        try:
            ...
            for col_index in range(2, max_col + 1):  # Start from column 2 (skip column A)
                ...
                print(f"  Retrieved {len(stmt_data)} income statement items for '{statement_date}'")
            ...
            print(f"\n✅ Income Statement Import completed successfully")
            print(f"📊 Successfully processed {success_count} ticker columns")
            
        except Exception as e:
            print(f"❌ Error during income statement import: {str(e)}")
            import traceback
            traceback.print_exc()
            raise
    ...
    def _get_valid_items_and_populate_column_a(self, ws, wb):
        ...
        try:
            ...
            # Find income statements section
            income_stmt_col = self._find_or_create_settings_section(settings_ws, "income statements")
            
            # Get/create INDEX columns
            default_col = income_stmt_col
            config_col = income_stmt_col + 1
...
def import_income_statements():
    """Import income statement data from Parquet to Excel - Called by VBA"""
    try:
        importer = IncomeStatementImporter()
        importer.import_data()
    except Exception as e:
        print(f"Error in import_income_statements: {str(e)}")
        raise
