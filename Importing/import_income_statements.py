...
- Parquet files: Income statement data from Yahoo Finance (long format)
- Terminal window settings: Selected income statement line items
...
# Default Yahoo-style order template for income statement line items
YAHOO_INCOME_STATEMENT_ORDER = [
    "Operating Revenue",
    "Total Revenue",
    "Cost Of Revenue",
...
class IncomeStatementImporter:
        ...
        try:
            ...
            for col_index in range(2, max_col + 1):  # Start from column 2 (skip column A)
                ...
                if not required_cols.issubset(set(df.columns)):
                    print(f"  Invalid income statement format for {ticker}")
    ...
    def _get_valid_items_and_populate_column_a(self, ws, wb):
        ...
        line_items = self._get_selected_items_from_terminal_settings("incomeStatement", default_items)
    ...
    def _apply_yahoo_order(self, available_items):
        ...
        ordered = [item for item in YAHOO_INCOME_STATEMENT_ORDER if item in available_items]
...
@xw.sub
def import_income_statements(workbook_path=None):
