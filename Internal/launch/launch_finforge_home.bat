@echo off
REM FinForge Home Launcher
REM This script launches the Electron-based FinForge workspace window
...
set "LAUNCH_LOG=%PROJECT_ROOT%\Temporary\launch_home_diagnostics.log"
echo ==================================================>>"%LAUNCH_LOG%"
echo [%date% %time%] FinForge home launch attempt>>"%LAUNCH_LOG%"
...
if exist "%ELECTRON_EXE%" (
    ...
    echo ERROR: Electron is not installed for FinForge Workspace.
    echo.
    echo Run the following from the ElectronHome folder:
    echo   npm install
    echo.
    echo Then run Internal\launch\launch_finforge_home.bat again.
    echo.
    pause
)